...
- 发现训练误差一直在减少，但未收敛；测试误差首先会下降然后一直波动，与第一问**图 1.3** 比较发现，训练误差减少的较慢，训练结果较差，在 150 万次后无法得到最优的参数
...
- 与第一问**图1.3** 进行比较发现，收敛需要的步数略小于前者，且最终的测试误差低于前者，也没有出现反弹的过拟合情况。
...
SGD 的优点在于当数据集部分数据冗余时，避免每次参数更新时对相似的样例进行梯度重复计算，而是**用样本中的一个例子来近似我所有的样本**，因此，SGD速度会更快（迭代步数可能不会减少，但每次迭代的计算时间减少）。

然而 SGD 的缺点在于，随机剃度下降是不会收敛到最优点的，它总是在最优点附近跳来跳去，并不是每次迭代都向着整体最优化方向，但是大的整体的方向是向全局最优解的，最终的结果往往是在全局最优解附近。即使我们到达了最优点，它依然会跳动，因为对于随机的样本来说，这些少数的样本在最优点的梯度也未必是0（整体的梯度是0）。
